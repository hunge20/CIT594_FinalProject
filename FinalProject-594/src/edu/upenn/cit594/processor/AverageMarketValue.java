...
public class AverageMarketValue extends PropertiesStrategy {
	
    //for memoization
    private Map<Integer, Integer> results = new HashMap<>();
    
	public AverageMarketValue(PropertyCsv propertyCsv) {
		super(propertyCsv);
	}
	
    //memoization
	public int getAverageMarketValue(int input){
	...
	public int doCalculation(int zipcode) {
		if (averageMarketValue.containsKey(zipcode)) {
			return averageMarketValue.get(zipcode);
		} else {
			...
			for (PropertyValues house: houses) {
				values.add(house.getMarketValue());
			}

			int average = doAverage(values);
			averageMarketValue.put(zipcode, average);
			return average;
		}
	}
	...
	public int doCalculation(int zipcode) {
		if (averageMarketValue.containsKey(zipcode)) {
            ...
            averageMarketValue.put(zipcode, avgMktVal);
            return avgMktVal;
        }
	}
	 */
